...
export async function fetchPolygonPrices(symbols) {
  console.log('[Polygon] POLYGON_API_KEY:', process.env.POLYGON_API_KEY ? 'defined' : 'undefined')
  const entries = await Promise.all(
    symbols.map(async (raw, idx) => {
      const sym = raw.toUpperCase()
      try {
        const url = `https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers/${encodeURIComponent(sym)}?apiKey=${process.env.POLYGON_API_KEY}`
        if (idx === 0) console.log('[Polygon] fetching URL:', url)
        const res = await fetch(url)
        if (!res.ok) return [sym, { price: null, dailyBar: null }]
        const text = await res.text()
        if (idx === 0) console.log('[Polygon] raw response body:', text)
